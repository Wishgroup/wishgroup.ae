...
const GrassShaderMaterial = {
  uniforms: {
    ...
    grassColor: { value: new THREE.Color(0.35, 0.55, 0.25) },
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      color.r += positionVariation * 0.1 + windVariation * 0.05;
      color.g += positionVariation * 0.15 + windVariation * 0.08;
      color.b += positionVariation * 0.05;
      
      // Height-based color gradient (darker at base, lighter at top)
      float heightGradient = mix(0.75, 1.15, pow(vHeight, 0.7));
...
const createGrassBladeGeometry = () => {
  const segments = 6; // More segments for smoother curves
  const width = 0.06;
  const height = 0.4;
